        ...
        for num in range(0, row):
            for x in range(8):
                try:
                    mode = (
                        curses.A_REVERSE
                        if x + ((num + self.offset) * 8)
                        in [0x3F00, 0x3E00, 0x3C00, 0x3800, 0x3FFF]
                        else curses.A_NORMAL
                    )
                    value = f"{self.data[x + ((num + self.offset) * 16)]:04X}"
                    stdscr.addstr(num + 3, 15 + (x * 8), f"{value[:2]}", mode)
                    stdscr.addstr(num + 3, 15 + (x * 8 + 4), f"{value[2:]}", mode)
                except:
                    pass
